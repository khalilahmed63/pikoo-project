...
import { setdiscountTmpId } from "../../redux/features/mobileInputs/formDataSlice";
...
const Step3 = () => {
  ...
  const { companyName, platform, platformCountry } =
    useSelector(selectFromData);
  ...
  const step3 = (haveToBuy: any) => {
    client
      .post(
        "/api/v1/discount/step",
        {
          step: "DISCOUNT_CREATION_STEP_GENERAL_INFO",
          info: {
            name: companyName,
            platform,
            platformCountry,
          ...
          cardDiscountInfo: {
            type: "DISCOUNT_TYPE_ITEM_ACCUMULATION",
            haveToBuy,
          },
        },
        {
      ...
      .then((res: any) => {
        if (res.status) {
          dispatch(setdiscountTmpId(res.data));
          router.push("/discount-create/step-1");
        }
      })
      .catch((err: any) => {
        setLoading(false);
  ...
  const step3Schema = yup.object().shape({
    haveToBuy: yup.number().required("Enter a Number"),
  });
  // yup schema
          ...
          <div className="max-w-md w-full sm:w-96 bg-white rounded-lg  shadow-md ">
            ...
            <div className="sm:pt-0 px-10 sm:px-10 md:px-8">
              <form onSubmit={formik.handleSubmit} method="post">
                <p className="font-bold text-xs mt-4">{`${t(
                  "common:titlepage"
                )}`}</p>
                <p className="text-xs">{`${t("common:expalination")}`}</p>
                  ...
                  <div>
                    <div className=" ">
                      <input
                        ...
                        value={formik.values.haveToBuy}
                      />
                    </div>
                    <p className="block text-xs text-center p-1 font-medium text-red-600">
                      {formik.errors.haveToBuy && formik.touched.haveToBuy && (
                        <div className="block text-xs font-medium text-red-600">
                          {formik.errors.haveToBuy}
                        </div>
                      )}
                    </p>
                  </div>
                </div>
                <p className="block text-center text-xs font-medium text-red-600">
                  {message}
                </p>
                {loading ? (
